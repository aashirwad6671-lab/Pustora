...
export class VoiceService {
  /**
   * Transcribes standard browser Blob audio recording using OpenAI Whisper API.
   */
  static async transcribeAudio(audioBlob: Blob): Promise<ApiResponse<string>> {
    try {
      if (!OPENAI_API_KEY) {
        return { data: null, error: 'OpenAI API key not configured in environment variables.', status: 400 };
      }
...
Analyze the query and determine their intent, then output a strict JSON object matching the VoiceAgentIntent type.
...
`;
   ...
   * Returns standard mp3 audio base64 URI.
   */
